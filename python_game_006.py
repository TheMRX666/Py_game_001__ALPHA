...
class Hero(Character):
    ...
    def skill_punch(self):
        self.skill_punch = (self.power*self.level)*1.5
        self.stamina -=10
        if self.sword == True:
            self.skill_punch = self.skill_punch*1.7
        print(f"You punch damage - {self.skill_punch}")
        return self.skill_punch
...
if __name__ == "__main__":
    ...
    player_1.punch()
    player_1.show_stat()
    skill_punch_damage = player_1.skill_punch()
    ...
    """)
            ...
            if dunge__level__start == 1:
                ...
                while pvp_index <= 5:
                    player_1.show_stat
                    slime_mob.show_stat()
                    cout_exp = 15*slime_mob.selflevel()
                    dunge_event = input("Enter Punch or SuperPunch\n")
                    ...
                    elif dunge_event == "SuperPunch":
                        slime_mob.dunge_get_damage(skill_punch_damage)
                        player_get_to_damage = slime_mob.dunge_mob_punch()
                        player_1.dunge__get__damage(player_get_to_damage)
                    else:
                        print("You missed")
                    ...
                    if slime_mob.dunge_get_damage(0) <= 0:
                        print("You win!!!")
                        player_1.add_exp(cout_exp)
                        player_1.add_money(100)
                        break
                    player_1.player_dead()
                dunge__level__start +=1
                player_1.player_dead()
            elif dunge__level__start == 2:
                wolf_mob = Mobs("Wolf", 3)
                pvp_index = 0
                while pvp_index <= 5:
                    player_1.show_stat
                    wolf_mob.show_stat()
                    cout_exp = 15*wolf_mob.selflevel()
                    dunge_event = input("Enter Punch or SuperPunch\n")
                    if dunge_event == "Punch":
                        wolf_mob.dunge_get_damage(punch_damage)
                        player_get_to_damage = wolf_mob.dunge_mob_punch()
                        player_1.dunge__get__damage(player_get_to_damage)
                    elif dunge_event == "SuperPunch":
                        wolf_mob.dunge_get_damage(skill_punch_damage)
                        player_get_to_damage = wolf_mob.dunge_mob_punch()
                        player_1.dunge__get__damage(player_get_to_damage)
                    else:
                        print("You missed")
                    ...
                    if wolf_mob.dunge_get_damage(0) <= 0:
                        print("You win!!!")
                        player_1.add_exp(cout_exp)
                        player_1.add_money(100)
                        break
...
EnergyPotion  -  Buy 1 Stamina Potion. Price 400$
            """)
